...
def validate(val_loader, net, classifier, classifier_opti, classifier_criterion):
        ...
        for i, data in enumerate(val_loader):
            ...
            for j in range(inps_batch.size(0)):
                ...
                for k in range(inps_batch.size(1)):
                    ...
                    classifier_opti.zero_grad()
                    classi_out = classifier(feat_flat)
                    soft_out = F.softmax(classi_out, 1).max(1)[1]
                    loss = classifier_criterion(true_flat, soft_out)
                    loss.backwards()
                    classifier_opti.step()
                    # for c in range(num_known_classes):
