...
#     net = nn.DataParallel(net)
    ...
    train_loader = DataLoader(train_set, batch_size=None, num_workers=args['num_workers'], shuffle=True)

    val_set = list_dataset.ListDataset(dataset_name, 'Val', (args['h_size'], args['w_size']), 'statistical', hidden, overlap=True, use_dsm=True)
...
#     optimizer = optim.SGD(net.parameters(), lr=args['lr'], momentum=args['momentum'], weight_decay=args['weight_decay'])
    ...
    for epoch in range(curr_epoch, args['epoch_num'] + 1):
        ...
        if epoch % args['test_freq'] == 0:
            ...
            torch.save(net.state_dict(), os.path.join(ckpt_path, exp_name, 'model_latest.pth'))
            torch.save(optimizer.state_dict(), os.path.join(ckpt_path, exp_name, 'opt_latest.pth'))
            
            # Computing test.
            testmodel(val_loader, net, optimizer, criterion, epoch, num_known_classes, num_unknown_classes, hidden, args, 10, True, True) #epoch % args['save_freq'] == 0)
...
def testmodel(test_loader, net, optimizer, criterion, epoch, num_known_classes, num_unknown_classes, hidden, args, num_minibatches, save_images, save_model):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader):
            ...
            for j in range(num_minibatches): #inps_batch.size(0)
                
                print('    Test MiniBatch %d/%d' % (j + 1, num_minibatches))
                sys.stdout.flush()
            ...
            loss.append(minibatch_loss/num_minibatches)
        ...
        for i, data in enumerate(test_loader):
            ...
            for j in range(inps_batch.size(0)):
                ...
                for k in range(inps_batch.size(1)):
                    ...
                    minibatch_loss += criterion(labs, prds).item()
